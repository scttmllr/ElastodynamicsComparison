...
//	void assemble_explicit_system();
    ...
    void assemble_momentum_system();
    void assemble_uv_system();
...
template <int dim>
void ElasticProblem<dim>::assemble_momentum_system ()
{
    ...
    for (; cell!=endc; ++cell)
 	{
        ...
        fe_values[disp].get_function_gradients(old_displacement, grad_u);
 ...
 }//assemble_momentum_matrix
    
template <int dim>
void ElasticProblem<dim>::assemble_uv_system ()
{
    // Zero matrices and vectors
    system_rhs = 0.;
    lumped_mass_matrix = 0.;
    ...
    std::vector<Tensor<1,dim> > local_v(n_q_points);
    ...
    for (; cell!=endc; ++cell)
    {
        ...
        fe_values[disp].get_function_values(velocity, local_v);
    ...
    for (; cell!=endc; ++cell)
 	{
        ...
        fe_values[disp].get_function_values(displacement, u);
        fe_values[disp].get_function_values(velocity, v);
        fe_values[disp].get_function_gradients(displacement, grad_u);
...
void ElasticProblem<dim>::run (std::string time_integrator, int nx, int ny, int nz)
{
    ...
    for(unsigned int step=0; step<n_timesteps; ++step)
    {
        ...
        for(unsigned int dof=0; dof<dof_handler.n_dofs(); ++dof)
        {
            displacement[dof] += system_rhs[dof]/lumped_mass_matrix[dof];
        }
